...
def call_llm_generate_selectors(
    ...
    system = (
        "You are an expert web automation engineer. Your task is to SELECT the single best selector "
        "from a provided list of candidates that matches the user's intent and the failed selector. "
        "You will be given:\n"
        "1. A failed selector (for context)\n"
        "2. A description of the intended action (Use of selector)\n"
        "3. A list of candidate selectors found in the DOM\n"
        "4. Visual context (optional)\n\n"
        "Your Goal: Identify the selector from the candidate list that is most robust, stable, and relevant "
        "to the intended action.\n\n"
        "CRITICAL INSTRUCTIONS:\n"
        "1. Analyze the 'Failed Selector' pattern. The correct selector often shares structural similarities "
        "or attributes with the failed one, even if some parts (like dynamic IDs) have changed.\n"
        "2. Pay close attention to the 'Purpose (Use of selector)'. The chosen selector MUST enable this "
        "specific action (e.g., if the purpose is 'click login', choose the button, not the wrapper).\n"
        "3. Prefer attributes like 'data-testid', 'aria-label', or specific text over generic classes or complex XPaths.\n\n"
        "Output Format (JSON):\n"
        "{\n"
        "  \"candidates\": [\"<BEST_SELECTOR>\"],\n"
        "  \"confidence\": [<CONFIDENCE_SCORE_0_TO_1>],\n"
        "  \"explanations\": [\"<BRIEF_REASONING>\"]\n"
        "}\n"
        "NOTE: Return ONLY the best selector in the 'candidates' list."
    )

    if dom_data and isinstance(dom_data, list):
        # Format candidates as a numbered list
        candidates_list = "\n".join([f"{i+1}. {c}" for i, c in enumerate(dom_data)])
        ...
        Task:
        Analyze the candidates above and select the ONE best selector that matches the purpose.
        Return the result in the specified JSON format.
        """
    elif dom_data:
        # Fallback if dom_data is not a list (though it should be based on recent changes)
        ...
        Task:
        Generate/Select the best selector based on the DOM data provided.
        Return JSON.
        """
    else:
        user = f"""
        ...
        Task:
        Generate/Select the best selector based on the HTML provided.
        Return JSON.
        """
